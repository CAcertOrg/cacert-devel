...
function deUmlaut2($str) {
	return mb_ereg_replace(iconv("ISO-8859-1", "UTF-8", "�"), "Ae",
		 mb_ereg_replace(iconv("ISO-8859-1", "UTF-8", "�"), "Oe",
		 mb_ereg_replace(iconv("ISO-8859-1", "UTF-8", "�"), "Ue",
		 mb_ereg_replace(iconv("ISO-8859-1", "UTF-8", "�"), "ae",
		 mb_ereg_replace(iconv("ISO-8859-1", "UTF-8", "�"), "oe",
		 mb_ereg_replace(iconv("ISO-8859-1", "UTF-8", "�"), "ue",
		 mb_ereg_replace(iconv("ISO-8859-1", "UTF-8", "�"), "ss",
				 mb_convert_encoding($str, "UTF-8"))))))));
}
...
function csvize($str) {
	...
	$res = $db_conn->query("SELECT id, memid FROM gpg WHERE crt != ''");
	if (!$res) {
	...
	foreach ($keys as $key) {
		...
		$res = $db_conn->query("SELECT fname, mname, lname, suffix FROM users WHERE id = " . $key[1]);
		if (!$res) {
		...
		$res = $db_conn->query("SELECT email FROM email WHERE hash = '' AND memid = " . $key[1]);
		if (!$res) {
			echo "Query FROM email failed!\n";
			exit;
		}
		$addrs = array();
		while ($addr = $res->fetch_row()) {
