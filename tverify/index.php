...
//	phpinfo(); exit;
	...
	if($id == 1)
	{
		$email = $db_conn->escape_string(trim($_REQUEST["email"]));
		$password = $db_conn->escape_string(stripslashes(trim($_REQUEST["pword"])));
		$URL = $db_conn->escape_string(trim($_REQUEST["notaryURL"]));
		$CN = $db_conn->escape_string($_SESSION['_config']['CN']);
		$memid = intval($_SESSION['_config']['uid']);
		$user = $db_conn->query("select * from `users` where `id`='$memid'")->fetch_assoc();
		$tmp = $db_conn->query("select sum(`points`) as `points` from `notary` where `to`='$memid'")->fetch_assoc();
	...
	if($id == 1)
	{
		...
		if($db_conn->query($query)->num_rows <= 0)
		{
			...
			$db_conn->query($query);
			$tverify = $db_conn->insert_id;
			if($nofile == 0)
			{
				$filename = $photoid['tmp_name'];
				$newfile = $db_conn->real_escape_string('/www/photoid/'.$tverify.".".$ext);
	...
	if($id == 1 && $max == 50)
	{
		if($points > 0)
		{
			$db_conn->query("insert into `notary` set `from`='0', `to`='$memid', `points`='$points',
					`method`='Thawte Points Transfer', `when`=NOW()");
			fix_assurer_flag($memid);
		}
		$totalpoints = intval($tmp['points']) + $points;
		$db_conn->query("update `tverify` set `modified`=NOW() where `id`='$tverify'");
