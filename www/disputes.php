...
<?
	...
	if($type == "reallyemail")
	{
		...
		$hash = $db_conn->real_escape_string(trim($_SESSION['_config']['hash']));

		$res = $db_conn->query("select * from `disputeemail` where `id`='$emailid' and `hash`='$hash'");
		if($res->num_rows <= 0)
		{
		...
		if($action == "reject")
		{
			$db_conn->query("update `disputeemail` set hash='',action='reject' where `id`='".intval($emailid)."'");
			showheader(_("Email Dispute"));
		...
		if($action == "accept")
		{
			...
			$db_conn->query("update `disputeemail` set hash='',action='accept' where `id`='$emailid'");
			$rc = $db_conn->query("select * from `domains` where `memid`='$oldmemid' and `deleted`=0")->num_rows;
			$rc2 = $db_conn->query("select * from `email` where `memid`='$oldmemid' and `deleted`=0 and `id`!='$emailid'")->num_rows;
			$res = $db_conn->query("select * from `users` where `id`='$oldmemid'");
			$user = $res->fetch_assoc();
			if($rc == 0 && $rc2 == 0 && $_SESSION['_config']['email'] == $user['email'])
			{
				$db_conn->query("update `users` set `deleted`=NOW() where `id`='$oldmemid'");
	...
	if($type == "email")
	{
		...
		$hash = $db_conn->real_escape_string(trim(stripslashes($_REQUEST['hash'])));
		if($emailid <= 0 || $hash == "")
		{
			showheader(_("Email Dispute"));
		...
		if($res->num_rows <= 0)
		{
			$res = $db_conn->query("select * from `disputeemail` where `id`='$emailid' and hash!=''");
			if($res->num_rows > 0)
			{
				$row = $res->fetch_assoc();
				$db_conn->query("update `disputeemail` set `attempts`='".intval($row['attempts'] + 1)."' where `id`='" .$row['id']."'");
				showheader(_("Email Dispute"));
				if($row['attempts'] >= 3)
				{
					...
					$db_conn->query("update `disputeemail` set hash='',action='failed' where `id`='$emailid'");
				} else
		...
		$row = $db_conn->query("select * from `disputeemail` where `id`='$emailid'")->fetch_assoc();
	...
	if($type == "reallydomain")
	{
		...
		$res = $db_conn->query("select * from `disputedomain` where `id`='$domainid' and `hash`='$hash'");
		if($res->num_rows <= 0)
		{
		...
		if($action == "reject")
		{
			$db_conn->query("update `disputedomain` set hash='',action='reject' where `id`='$domainid'");
			showheader(_("Domain Dispute"));
			echo _("You have opted to reject this dispute and the request will be removed from the database");
			showfooter();
			exit;
		...
		if($action == "accept")
		{
			...
			$db_conn->query("update `disputedomain` set hash='',action='accept' where `id`='$domainid'");
			showfooter();
			exit;
		}
	}
	...
	if($type == "domain")
	{
		...
		$hash = trim($db_conn->real_escape_string(stripslashes($_REQUEST['hash'])));
		if($domainid <= 0 || $hash == "")
		{
			showheader(_("Domain Dispute"));
			echo _("Invalid request. Can't continue.");
		...
		if($res->num_rows <= 0)
		{
			$res = $db_conn->query("select * from `disputedomain` where `id`='$domainid' and hash!=''");
			if($res->num_rows > 0)
			{
				$row = $res->fetch_assoc();
				$db_conn->query("update `disputedomain` set `attempts`='".intval($row['attempts'] + 1)."' where `id`='" .$row['id']."'");
				showheader(_("Domain Dispute"));
				if($row['attempts'] >= 3)
				{
					...
					$db_conn->query("update `disputedomain` set hash='',action='failed' where `id`='$domainid'");
				} else
		...
		$_SESSION['_config']['hash'] = $hash;
		$row = $db_conn->query("select * from `disputedomain` where `id`='$domainid'")->fetch_assoc();
	...
	if($oldid == "1")
	{
		...
		$email = $db_conn->real_escape_string(trim(stripslashes($_REQUEST['dispute'])));
		if($email == "")
		{
			showheader(_("Email Dispute"));
		...
		$res = $db_conn->query("select 1 from `email`, `users` where `email`.`email`='$email' and `email`.`memid`=`users`.`id` and (`users`.`assurer_blocked`=1 or `users`.`locked`=1)");
		if($res->num_rows > 0)
		{
			showheader(_("Email Dispute"));
		...
		$res = $db_conn->query("select * from `disputeemail` where `email`='$email' and hash!=''");
		if($res->num_rows > 0)
		{
			showheader(_("Email Dispute"));
	...
	if($oldid == "2")
	{
		...
		$domain = $db_conn->real_escape_string(trim(stripslashes($_REQUEST['dispute'])));
		if($domain == "")
		{
			showheader(_("Domain Dispute"));
		...
		$res = $db_conn->query("select 1 from `domains`, `users` where `domains`.`domain`='$domain' and `domains`.`memid`=`users`.`id` and (`users`.`assurer_blocked`=1 or `users`.`locked`=1)");
		if($res->num_rows > 0)
		{
			showheader(_("Domain Dispute"));
	...
	if($oldid == "5")
	{
                $authaddy = $db_conn->escape_string(trim(stripslashes($_REQUEST['authaddy'])));
		...
		$domain = $db_conn->real_escape_string($_SESSION['_config']['domain']);
